...
@Component({
  selector: 'app-moneyallocatedadd',
  templateUrl: './moneyallocatedadd.component.html',
  styleUrls: ['./moneyallocatedadd.component.css']
})
export class MoneyallocatedaddComponent implements OnInit {
  remitform: FormGroup;
  errormessage:any={};
  loginerror: any;
  loading:any=false;
...
save():void{
  ...
  this.loginservice.user.getIdToken(/* forceRefresh */ true).then((idToken)=>{
    ...
    this.http.post<any | Error>('http://localhost:3030/api/allocate', set).subscribe((data)=>{
      alert(data.message);
      this.loading=false;
      this.router.navigate(['/admin', 'user', this.uid, this.uid, 'moneyallocated', this.uid])
    }, (error)=>{
      alert(error.error);
        this.loading=false;
    })

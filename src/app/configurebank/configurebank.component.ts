...
@Component({
  ...
  templateUrl: './configurebank.component.html',
  styleUrls: ['./configurebank.component.css']
})
export class ConfigurebankComponent implements OnInit {
  remitform: FormGroup;
  errormessage:any={};
  loginerror: any;
...
save():void{
  this.loginservice.user.getIdToken(/* forceRefresh */ true).then((idToken)=>{
    ...
    let set = {...hey, ...others}
    console.log(set);
    this.http.post<any | Error>('http://localhost:3030/api/fin', set).pipe(
      map(d=><any>{
        message:d.message
      }),
